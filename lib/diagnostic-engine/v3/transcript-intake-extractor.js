 ...
 */
/**
 * Build the full prompt config for client-side OpenRouter call (intake extraction).
 * Returns everything the browser needs to call OpenRouter directly.
 */
export function buildIntakePromptConfig(transcriptText) {
  const questionsPrompt = buildQuestionsPrompt();
  const systemPrompt = buildIntakeSystemPrompt(questionsPrompt);
  return {
    model: 'anthropic/claude-sonnet-4',
    systemPrompt,
    userMessage: `Extract intake form answers from the following discovery call transcript.\n\n<transcript>\n${transcriptText}\n</transcript>`,
    tools: [{
      type: 'function',
      function: {
        name: intakeExtractionTool.name,
        description: intakeExtractionTool.description,
        parameters: intakeExtractionTool.input_schema,
      },
    }],
    toolChoice: { type: 'function', function: { name: 'submit_intake_answers' } },
  };
}

/**
 * Parse and validate an OpenRouter response for intake extraction.
 * Exported so the server-side store endpoint can validate client-submitted results.
 */
export { parseIntakeResponse };
