 ...
 * Transcript Analyzer
 *
 * Processes discovery call transcripts using Claude (via OpenRouter) to extract
 ...
 * @param {string} options.model - Model to use via OpenRouter (default: 'anthropic/claude-sonnet-4')
...
export async function analyzeTranscript(transcriptText, options = {}) {
  ...
  if (!apiKey) {
    throw new Error('OPENROUTER_API_KEY is required for transcript analysis');
  }
...
/**
 * Parse and validate an OpenRouter response for competency analysis.
 ...
 */
export function buildAnalyzePromptConfig(transcriptText) {
  const rubricText = buildRubricReference();
  const systemPrompt = buildSystemPrompt(rubricText);
  return {
    model: 'anthropic/claude-sonnet-4',
    systemPrompt,
    userMessage: `Analyze the following discovery call transcript and extract competency assessments.\n\n<transcript>\n${transcriptText}\n</transcript>`,
    tools: [{
      type: 'function',
      function: {
        name: extractionTool.name,
        description: extractionTool.description,
        parameters: extractionTool.input_schema,
      },
    }],
    toolChoice: { type: 'function', function: { name: 'submit_assessments' } },
  };
}
...
async function callClaude({ model, apiKey, system, messages, tools }) {
  ...
  // OpenRouter uses OpenAI-compatible format: system prompt is a message
  const allMessages = [
  ...
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      tools: openAiTools,
      tool_choice: { type: 'function', function: { name: 'submit_assessments' } },
    }),
  });
...
function parseExtractionResponse(response) {
  // OpenAI-compatible format: response.choices[0].message.tool_calls
  const toolCalls = response.choices?.[0]?.message?.tool_calls;
  if (!toolCalls || toolCalls.length === 0) {
    return [];
  }

  const call = toolCalls.find((tc) => tc.function?.name === 'submit_assessments');
  if (!call) {
    return [];
  }
