...
export function resolvePower10FromSnapshot(snapshot) {
  ...
  if (!rrc) {
    ...
    const windowStartMrr = rrc.balances?.window_start?.mrr;

    // TTM path requires a positive window_start.mrr balance from 13 months ago.
    // When absent (legacy snapshots, or new customers without 12mo history) we
    // fall back to single-month math against start_of_period.mrr.
    const useTtm = windowStartMrr != null && windowStartMrr > 0;
    const denominator = useTtm ? windowStartMrr : startMrr;
    const cadence = useTtm ? 'trailing_12m' : 'single_month';

    if (!denominator || denominator <= 0) {
      const reason = 'recurring_revenue_changes (no usable MRR denominator)';
      ...
      // TTM: sum across all 12 monthly rows. Single-month: only rows for `period`.
      const relevantRows = useTtm
        ? (rrc.monthly || [])
        : (rrc.monthly || []).filter(r => r.month === period);
      const retentionLoss = relevantRows
        .filter(r => r.phase === 'RETENTION' && r.recurring_revenue_actuals < 0)
        .reduce((s, r) => s + r.recurring_revenue_actuals, 0); // negative number
      const retentionNet = relevantRows
        .filter(r => r.phase === 'RETENTION')
        .reduce((s, r) => s + r.recurring_revenue_actuals, 0);
      const expansionNet = relevantRows
        .filter(r => r.phase === 'EXPANSION')
        .reduce((s, r) => s + r.recurring_revenue_actuals, 0);

      const grossChurn = -retentionLoss / denominator;
      const grr = 1 - grossChurn;
      const nrr = (denominator + retentionNet + expansionNet) / denominator;
      ...
      // For single-month cadence, gross churn is a monthly ratio — annualize
      // (×12) before applying the 5%/10% healthy/careful thresholds.
      // For TTM cadence the value is already annual-equivalent — apply directly.
      const grossChurnForBucket = useTtm ? grossChurnRounded : grossChurnRounded * 12;
      ...
      out.D5_gross_churn = {
        ...
        stale,
        cadence,
        performance: bucketByThreshold(grossChurnForBucket, { healthyAt: 0.05, carefulAt: 0.10, lowerIsBetter: true }),
      };
      out.D5_grr = {
