...
/**
 * Tech stack categories (from snapshot.tech_stack flat object) → competency mapping.
 * snapshot.tech_stack shape: { crm: 'Salesforce', call_recording: 'Gong', ... }
 * A non-empty string value indicates the tool is in place.
 */
const TECH_STACK_TO_COMPETENCY = [
  { competency: 'SY-5', keys: ['forecasting', 'revenue_intelligence', 'rev_ops'], label: 'Forecasting platform' },
  { competency: 'SY-6', keys: ['cs_platform', 'customer_success'],                label: 'CS platform' },
  { competency: 'SY-7', keys: ['call_recording', 'conversation_intelligence'],    label: 'Conversation intelligence' },
];

/**
 * Count integration / data infrastructure tools present in tech_stack —
 * proxy for SY-2 (integration architecture).
 */
...
/**
 * Count enrichment / intelligence tools — proxy for SY-4 (data enrichment).
 * Falls back to scanning context_graph for known vendor names.
 */
function countEnrichmentTools(techStack, contextGraph) {
  let count = 0;
  if (techStack && typeof techStack === 'object') {
    ...
    if (isFilled(techStack.product_analytics)) count += 1;
  }
  // Context graph fallback — look for known enrichment vendor names in node lists
  const enrichmentVendors = /zoominfo|clearbit|apollo|cognism|lusha|clay|6sense|demandbase/i;
  if (contextGraph && Array.isArray(contextGraph.tools)) {
    for (const t of contextGraph.tools) {
      const name = t?.name || t?.vendor || '';
      if (enrichmentVendors.test(name)) count += 1;
    }
  }
  return count;
}
...
export function mapSnapshotToCompetencyScores(snapshot) {
  ...
  // RP-5: Pipeline coverage ← pipeline value vs. quota (target: 3-4x)
  // Vasco may surface this directly under volume_metrics, snapshot.metrics, or as
  // computed fields (open_pipeline / quota). Try each in priority order.
  // Departments: sales, cs (revenue)
  ...
  if (pipelineCoverage != null) {
    scores['RP-5'] = {
      ...
      rationale: `Pipeline coverage: ${pipelineCoverage.toFixed(2)}x quota (target ≥3x)`,
    };
  }

  // RP-6: Forecast accuracy ← forecast vs. actuals variance (closer to 1.0 is better)
  // Score window: green if within ±10% of target, yellow within ±25%, red beyond.
  // Departments: sales
  ...
  // SY-2 (Integration architecture), SY-4 (Data enrichment),
  // SY-5/SY-6/SY-7 (Forecasting / CS / Conversation intel platforms)
  // ← derived from snapshot.tech_stack and context_graph
  // Departments: matches each competency's defined departments (handled by caller)
  const techStack = snapshot.tech_stack || null;
  const contextGraph = snapshot.context_graph || null;

  if (techStack || contextGraph) {
    // SY-5/SY-6/SY-7 — single tool presence is a 5; absence is 1.
    for (const { competency, keys, label } of TECH_STACK_TO_COMPETENCY) {
      if (!techStack) continue;
      const hit = keys.find(k => isFilled(techStack[k]));
      if (hit) {
        scores[competency] = {
          score: 5,
          rationale: `${label}: ${techStack[hit]} (from Vasco tech stack)`,
        };
      } else if (Object.keys(techStack).length > 0) {
        // Snapshot lists tech stack categories but this one is empty → confirmed gap
        scores[competency] = {
          score: 1,
          rationale: `${label}: no tool detected in Vasco tech stack`,
        };
      }
    }

    // SY-2 — integration architecture: ≥2 integration tools = 5, 1 = 3, 0 = 1
    if (techStack) {
      const integrationCount = countIntegrationTools(techStack);
      scores['SY-2'] = {
        score: integrationCount >= 2 ? 5 : integrationCount === 1 ? 3 : 1,
        rationale: `Integration tools detected: ${integrationCount} (data warehouse / iPaaS / ETL)`,
      };
    }

    // SY-4 — data enrichment: ≥2 enrichment sources = 5, 1 = 3, 0 = 1
    const enrichmentCount = countEnrichmentTools(techStack, contextGraph);
    if (techStack || (contextGraph && Array.isArray(contextGraph.tools))) {
      scores['SY-4'] = {
        score: enrichmentCount >= 2 ? 5 : enrichmentCount === 1 ? 3 : 1,
        rationale: `Enrichment / intelligence tools detected: ${enrichmentCount}`,
      };
    }
  }
