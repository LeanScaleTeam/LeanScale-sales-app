...
export function generateExecutiveSummaryV3(roadmap, pillarScores, customerName) {
  const overallScore = pillarScores
    ? Object.values(pillarScores).reduce((s, v) => s + (Number(v) || 0), 0) / Math.max(1, Object.values(pillarScores).length)
    : 0;
  ...
  const pillarSummary = pillarScores
    ? Object.entries(pillarScores)
        .map(([key, score]) => `${PILLAR_LABELS_SOW[key] || key}: ${(Number(score) || 0).toFixed(1)}/5.0`)
        .join(', ')
    : '';
