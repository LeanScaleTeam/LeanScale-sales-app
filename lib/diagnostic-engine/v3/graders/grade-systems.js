 ...
 * has_marketing_automation_package (installed MAP tool), email_template_count
 */
function gradeSY2(s) {
  const emails = num(s, 'marketing_email_count');
  const forms = num(s, 'form_count');
  const lists = num(s, 'total_active_lists');
  const hasMapPackage = bool(s, 'has_marketing_automation_package');
  const templateCount = num(s, 'email_template_count');

  let score;
  if (emails > 50 && forms > 5 && lists > 10) {
    score = 5;
  } else if ((emails > 20 && forms > 2) || (hasMapPackage && templateCount > 10)) {
    score = 4;
  } else if (emails > 0 || hasMapPackage) {
    score = 3;
  } else if (forms > 0 || templateCount > 5) {
    score = 2;
  } else {
    score = 1;
  }
  ...
  if (hasMapPackage) {
    signals.push({
      name: 'Marketing automation platform',
      value: 'MAP tool detected (installed package)',
      impact: 'positive',
      source: 'api',
    });
  }
  ...
  if (templateCount > 0) {
    signals.push({
      ...
      value: `${templateCount} active templates`,
      impact: templateCount > 10 ? 'positive' : 'neutral',
      source: 'api',
    });
  }
  if (signals.length === 0) {
 ...
 * has_cs_platform_installed (Gainsight, ChurnZero, etc.)
 */
function gradeSY4(s) {
  ...
  const csPlatform = bool(s, 'has_cs_platform_installed');

  const indicatorCount = [ticketCustomized, csHandoff, healthScoring, csPlatform].filter(Boolean).length;
  ...
  if (indicatorCount >= 4) {
    ...
    score = 4;
  } else if (ticketCustomized || csPlatform) {
    score = 3;
  ...
  if (csPlatform) {
    signals.push({
      name: 'CS platform',
      value: 'Customer success tool detected (installed package)',
      impact: 'positive',
      source: 'api',
    });
  }
 ...
 * Checks intakeAnswers.A5, signals.has_partner_pipeline, has_prm_tool_intake
...
function gradeSY5(s, intakeAnswers) {
  ...
  const hasPartners = intake.A5 && intake.A5 !== 'No';
  const hasPrmTool = !!intake.has_prm_tool_intake;

  let score;
  if (hasPrmTool && hasPartnerPipeline) {
    score = 5;
  } else if (hasPartners && hasPartnerPipeline) {
    score = 5;
  } else if (hasPrmTool) {
    score = 4;
  ...
  if (hasPrmTool) {
    signals.push({
      name: 'PRM tool (intake)',
      value: 'Partner management platform reported',
      impact: 'positive',
      source: 'intake',
    });
  }
  ...
  if (hasPartners && !hasPrmTool && !hasPartnerPipeline) {
    signals.push({
      name: 'Partner program (intake)',
      value: 'Has partners, no dedicated platform',
      impact: 'neutral',
      source: 'intake',
    });
  }
  if (!hasPartnerPipeline && !hasPartners && !hasPrmTool) {
    signals.push({
