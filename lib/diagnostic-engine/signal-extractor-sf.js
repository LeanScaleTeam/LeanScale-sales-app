...
export function extractSalesforceSignals(metadata) {
  ...
  // Salesforce automations use MasterLabel/Label (flows) or Name (workflowRules)
  const allAutomations = [
    ...flowList.map((f) => ({ name: f.MasterLabel || f.Label || f.Name || '', description: f.Description || '', active: f.Status === 'Active' })),
  ...
  return {
    ...
    // Handle both aggregate rows (sum cnt) and individual records (count rows)
    record_type_count: recordTypeList.reduce((sum, rt) => sum + (rt.cnt || 1), 0),
    ...
    // ── v3 Phase 1: Field Pattern Detections (from existing metadata) ──

    // Methodology fields on Opportunity (PR-6)
    methodology_field_count: countMethodologyFields(opportunityFields),
    has_sales_methodology_fields: countMethodologyFields(opportunityFields) > 0,

    // Required deal fields via stage-gated validation rules (PR-6)
    has_required_deal_fields: detectStageGatedValidation(valRuleList),

    // Renewal tracking on Opportunity (PR-3)
    has_renewal_tracking: detectFieldPattern(
      opportunityFields,
      /renewal|contract.?end|expir|subscription.?end|term.?date|renewal.?date/i
    ),

    // Health scoring on Account (PR-3, SY-4)
    has_health_scoring: detectFieldPattern(
      accountFields,
      /health.?scor|risk.?scor|sentiment|nps|csat|satisfaction|customer.?score/i
    ),

    // ABM/target account fields on Account (PR-8)
    has_abm_fields: detectFieldPattern(
      accountFields,
      /target.?account|tier|icp|ideal.?customer|abm|account.?score|intent.?score|fit.?score|engagement.?score/i
    ),

    // Commission/comp fields on Opportunity (PE-4)
    has_commission_fields: detectFieldPattern(
      opportunityFields,
      /commission|comp.?plan|quota|spiff|accelerator|payout|attainment|variable.?comp/i
    ),
  };
}
...
function extractRoleNames(roles) {
  ...
  return roles.map((r) => r.Name || r.DeveloperName).filter(Boolean);
}
...
function groupRecordTypes(recordTypes) {
  ...
  for (const rt of recordTypes) {
    ...
    if (obj) {
      // Handle both aggregate rows (cnt field) and individual records
      groups[obj] = (groups[obj] || 0) + (rt.cnt || 1);
    }
  }
  return groups;
}
...
function countOutboundFlows(flows) {
  ...
  return flows.filter((f) => outboundPattern.test(f.MasterLabel || f.Label || f.Name || '')).length;
}

// ── v3 Phase 1: Field Pattern Detection Helpers ──

/**
 * Check if any field in the list matches a regex pattern (by name or label).
 */
function detectFieldPattern(fields, pattern) {
  if (!Array.isArray(fields)) return false;
  return fields.some((f) => pattern.test(f.name || '') || pattern.test(f.label || ''));
}

/**
 * Count Opportunity fields matching sales methodology naming patterns.
 * (MEDDIC, BANT, SPICED, etc.)
 */
function countMethodologyFields(fields) {
  if (!Array.isArray(fields)) return 0;
  const pattern = /meddic|meddpicc|bant|champion|decision.?crit|decision.?process|paper.?process|implicated.?pain|economic.?buyer|compelling.?event|next.?step|close.?plan|spiced|situation|pain|impact|critical.?event/i;
  return fields.filter((f) => pattern.test(f.name || '') || pattern.test(f.label || '')).length;
}

/**
 * Detect validation rules on Opportunity that gate fields by StageName.
 * This indicates enforced required fields at stage transitions (sales methodology).
 */
function detectStageGatedValidation(validationRules) {
  if (!Array.isArray(validationRules)) return false;
  return validationRules.some((v) => {
    const entity = v.EntityDefinition?.QualifiedApiName || v.EntityDefinition || '';
    const isOpp = /^Opportunity$/i.test(typeof entity === 'string' ? entity : '');
    if (!isOpp) return false;
    const text = `${v.ValidationName || ''} ${v.Description || ''} ${v.ErrorMessage || ''}`;
    return /StageName|stage.?gate|required.*field|field.*required/i.test(text);
  });
}
