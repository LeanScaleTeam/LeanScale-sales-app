 ...
 */
export async function upsertDiagnosticResult({ customerId, diagnosticType, processes, tools, power10Metrics, engagementOverrides, createdBy }) {
  if (!supabaseAdmin) return null;
  if (!VALID_TYPES.includes(diagnosticType)) return null;
  ...
  try {
    const row = {
      ...
      // Clear v2 fields so page reload detects this as v1
      version: null,
      items: null,
      scores: null,
    ...
    // Only include power10_metrics if provided (avoids overwriting with empty on older clients)
    if (power10Metrics !== undefined) {
      row.power10_metrics = power10Metrics || [];
    }
    if (engagementOverrides !== undefined) {
      row.engagement_overrides = engagementOverrides;
    }
