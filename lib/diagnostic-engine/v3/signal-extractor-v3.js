...
export function extractSignalsV3HubSpot(metadata) {
  ...
  const { properties, pipelines, workflows, forms, lists, owners, marketing_emails, sequences, tasks, meetings, calls, campaigns, goals, blog_posts, knowledge_articles } = metadata || {};
  ...
  // Blog/content counts (EN-1, EN-2)
  base.blog_post_count = blog_posts?.published_count || blog_posts?.total || 0;
  ...
  base.knowledge_article_count = knowledge_articles?.total || 0;

  // Dashboard/report counts (RP-1)
  // HubSpot doesn't expose dashboard counts via API; leave as 0
  ...
  base.campaign_count = campaignResults.length;
  base.has_abm_campaigns = campaignResults.some((c) =>
    /abm|target.*account|account.*based/i.test(c.name || c.type || '')
  );

  // Cadence types from meetings (RP-4 — enables grader fallback for HubSpot)
  base.cadence_types = {
    daily: cadenceMeetings.filter((m) => /daily|standup|stand.?up/i.test(m.properties?.hs_meeting_title || '')).length,
    weekly: cadenceMeetings.filter((m) => /weekly|WBR|1.?on.?1|one.?on.?one/i.test(m.properties?.hs_meeting_title || '')).length,
    monthly: cadenceMeetings.filter((m) => /monthly|pipeline.*review/i.test(m.properties?.hs_meeting_title || '')).length,
    quarterly: cadenceMeetings.filter((m) => /QBR|quarterly/i.test(m.properties?.hs_meeting_title || '')).length,
  };
  ...
  base.has_forecasting_config = goalResults.length > 0;
  base.goal_count = goalResults.length;

  // ── HubSpot-specific signal fixes ──

  // Gap 3: HubSpot IS the marketing automation platform — no external package needed
  base.has_marketing_automation_package = true;

  // Gap 1: Integration health proxy from workflow names (SY-7)
  // HubSpot doesn't have ConnectedApps/NamedCredentials; detect integrations from workflows
  const integrationWorkflows = workflowList.filter((w) =>
    /zapier|slack|stripe|intercom|zendesk|webhook|http.*action|salesforce|hubspot.*sync|make\.com|integromat|workato|tray\.io/i.test(w.name || '')
  );
  base.connected_app_count = Math.max(base.connected_app_count || 0, integrationWorkflows.length);
  base.integration_count = Math.max(base.integration_count || 0, integrationWorkflows.length);

  // Gap 8: Health scoring detection from contact/company properties (SY-4, PR-3)
  const allHsProps = [
    ...(properties?.contacts?.results || []),
    ...(properties?.companies?.results || []),
  ];
  base.has_health_scoring = allHsProps.some((p) =>
    /health.*score|account.*health|cs.*health|customer.*health/i.test(p.name || p.label || '')
  );

  // Gap 4: Signal aliases (PR-9 grader expects these, base extractor uses different names)
  ...
  // Gap A: total_active_lists alias (SY-2 grader reads this, v2 extractor sets dynamic/static separately)
  base.total_active_lists = (base.dynamic_list_count || 0) + (base.static_list_count || 0);

  // Gap B: Sequence count (SY-3 — sequences downloaded but never extracted)
  const sequenceResults = sequences?.results || (Array.isArray(sequences) ? sequences : []);
  base.sequence_count = sequenceResults.length;
  base.has_sequences = base.sequence_count > 0;

  // Gap C: Sales methodology field detection (PR-6)
  const dealProps = properties?.deals?.results || [];
  const methodologyPattern = /meddic|meddicc|meddpicc|bant|spiced|spin|challenger|sandler|value.?sell|command.?of|force.?manage|methodology|qualification.?framework|decision.?criteria|decision.?process|pain.?point|champion|economic.?buyer/i;
  base.methodology_field_count = dealProps.filter((p) => methodologyPattern.test(p.name || p.label || '')).length;

  // Gap D: Commission field detection (PE-4)
  const commissionFieldPattern = /commission|comp.?plan|variable.?comp|quota|spiff|bonus|incentive|payout|accelerat|kicker/i;
  base.has_commission_fields = dealProps.some((p) => commissionFieldPattern.test(p.name || p.label || ''));
  // Commission tool detection from properties (PE-4)
  const commissionToolPattern = /xactly|captivateiq|spiff|performio|everstage|varicent|commissionly/i;
  const allPropsFlat = [
    ...allHsProps,
    ...dealProps,
  ];
  base.has_commission_tool = allPropsFlat.some((p) => commissionToolPattern.test(p.name || p.label || ''));

  // Gap E: Renewal tracking detection (PR-3)
  const ticketProps = properties?.tickets?.results || [];
  const renewalPattern = /renewal|contract.?end|expir|subscription.?end|renewal.?date|contract.?date|term.?end/i;
  base.has_renewal_tracking = dealProps.some((p) => renewalPattern.test(p.name || p.label || ''))
    || ticketProps.some((p) => renewalPattern.test(p.name || p.label || ''));

  // Gap F: ABM field and tool detection (PR-8)
  const abmFieldPattern = /abm|target.?account|account.?tier|icp.?score|icp.?fit|ideal.?customer|account.?score|account.?grade/i;
  base.has_abm_fields = allPropsFlat.some((p) => abmFieldPattern.test(p.name || p.label || ''));
  const abmToolPattern = /6sense|demandbase|terminus|rollworks|triblio|madison.?logic/i;
  base.has_abm_tool = allPropsFlat.some((p) => abmToolPattern.test(p.name || p.label || ''));

  // CS platform detection from properties (SY-4)
  const csToolPattern = /gainsight|churnzero|vitally|totango|customer.?success|planhat|catalyst/i;
  base.has_cs_platform_installed = allPropsFlat.some((p) => csToolPattern.test(p.name || p.label || ''));
