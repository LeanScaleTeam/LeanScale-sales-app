...
export function runDiagnosticV3(
  ...
  // Multi-CRM support: caller can pass either a legacy string ('salesforce',
  // 'hubspot', 'attio', 'dual') or a crm_systems array. We derive a single
  // string for back-compat display fields and keep the array for new consumers.
  const { normalizeCrmSystems, deriveLegacyCrmType } = require('../crm-systems');
  const crmSystems = Array.isArray(crmTypeOrSystems)
    ? normalizeCrmSystems(crmTypeOrSystems)
    : normalizeCrmSystems(crmTypeOrSystems);
  const crmType = crmSystems.length > 0 ? deriveLegacyCrmType(crmSystems) : (typeof crmTypeOrSystems === 'string' ? crmTypeOrSystems : 'unknown');
  ...
  // Generate prioritized roadmap (with transcript project signals if available)
  const roadmap = generateRoadmap(allCompetencies, { projectSignals });

  // Collect actionable service IDs
  const actionableServices = collectActionableServiceIdsV3(allCompetencies);

  // Build company profile from intake. crm stays a single string for back-compat
  // (the results page UI calls .toLowerCase() on it). crmSystems carries the
  // canonical multi-system array for newer consumers.
