...
// ── Per-competency phase defaults (override pillar default) ──
// SY-1 (CRM core) stays at PILLAR_DEFAULT_PHASE['systems'] = FOUNDATION.
// Tool/platform competencies escalate out of FOUNDATION.
const COMPETENCY_DEFAULT_PHASE = {
  'SY-2': 'BUILD',   // Marketing automation platform
  'SY-3': 'BUILD',   // Sales engagement platform
  'SY-4': 'BUILD',   // CS / support platform
  'SY-5': 'OPTIMIZE', // Partner management platform
  'SY-6': 'OPTIMIZE', // Intelligence tools (enrichment, CI)
};
...
function assignPhase(project, priority) {
  ...
  // Get the primary pillar and primary competency (both by frequency)
  const pillarCounts = {};
  const compCounts = {};
  for (const comp of competencies) {
    pillarCounts[comp.pillar] = (pillarCounts[comp.pillar] || 0) + 1;
    compCounts[comp.id] = (compCounts[comp.id] || 0) + 1;
  }
  const primaryPillar = Object.entries(pillarCounts)
    .sort((a, b) => b[1] - a[1])[0]?.[0] || 'process';
  const primaryComp = Object.entries(compCounts)
    .sort((a, b) => b[1] - a[1])[0]?.[0];

  // Default phase: competency-specific override → pillar default
  let phase = (primaryComp && COMPETENCY_DEFAULT_PHASE[primaryComp])
    || PILLAR_DEFAULT_PHASE[primaryPillar]
    || 'BUILD';
