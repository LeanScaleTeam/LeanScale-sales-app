...
/**
 * Compute PE-4 API base score from commission/comp field and tool detection.
 * Weak signal — capped at 3 from API alone, transcript/consultant for 4+.
 *
 * Returns a 1-3 score or null if no commission signals detected.
 */
function computeCommissionScore(signals) {
  const hasFields = !!signals.has_commission_fields;
  const hasTool = !!signals.has_commission_tool;

  if (!hasFields && !hasTool) return null;
  if (hasFields && hasTool) return 3; // Both — strong signal, still capped
  if (hasTool) return 3;
  if (hasFields) return 2; // Fields only — weaker
  return null;
}
...
export function gradePeople(signals, intakeAnswers, transcriptScores, consultantScores) {
  // Pre-compute API base scores once
  const orgStructureBase = computeOrgStructureScore(signals);
  const commissionBase = computeCommissionScore(signals);

  return PEOPLE_COMPETENCIES.map((competency) => {
    const departments = {};
    const gradeSignals = [];

    // Determine API base score for this competency
    let apiBase = null;
    if (competency.id === 'PE-6' && orgStructureBase !== null) {
      apiBase = orgStructureBase;
      gradeSignals.push({
    ...
    } else if (competency.id === 'PE-4' && commissionBase !== null) {
      apiBase = commissionBase;
      const parts = [];
      if (signals.has_commission_fields) parts.push('commission fields on Opportunity');
      if (signals.has_commission_tool) parts.push('commission tool detected (installed package)');
      gradeSignals.push({
        name: 'Commission signals (CRM)',
        value: parts.join(', ') || 'Commission signals detected',
        impact: deriveImpact(commissionBase),
        source: 'api',
      });
    }
