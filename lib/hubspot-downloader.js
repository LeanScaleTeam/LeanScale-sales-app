...
export async function downloadAndStoreMetadata(customerId, portalId, accessToken) {
  ...
  const [properties, pipelines, workflows, forms, lists, owners, marketingEmails, sequences, dealAggregates, contactSources, tasks, meetings, calls, customObjectSchemas, campaigns, goals, blogPosts, knowledgeArticles] =
    await Promise.all([
      ...
      downloadBlogPosts(headers),
      downloadKnowledgeArticles(headers),
    ]);
...
/**
 * Download blog posts for content maturity signals (EN-1, EN-2, EN-5).
 * Requires `content` scope (optional — returns null if account lacks CMS/Marketing Pro+).
 */
async function downloadBlogPosts(headers) {
  try {
    const data = await fetchJSON(`${API_BASE}/cms/v3/blogs/posts?limit=100&sort=-publishDate`, headers);
    if (!data) return null;
    ...
    console.warn('HubSpot blog posts download error:', err.message);
    return null;
  }
}

/**
 * Download knowledge base articles via site search for enablement signals (EN-5).
 * Uses site search endpoint as workaround (no dedicated KB API).
 */
async function downloadKnowledgeArticles(headers) {
  try {
    const data = await fetchJSON(`${API_BASE}/cms/v3/site-search/search?type=KNOWLEDGE_ARTICLE&limit=100`, headers);
    if (!data) return null;
    return {
      total: data.total || 0,
    ...
    console.warn('HubSpot knowledge articles download error:', err.message);
    return null;
  }
}
