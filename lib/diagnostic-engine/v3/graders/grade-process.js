 ...
 * Uses has_required_deal_fields, has_stalled_deal_notification
 */
function gradePR6(s) {
  const gradeSignals = [];

  if (s.has_required_deal_fields === undefined && s.has_stalled_deal_notification === undefined) {
    return { score: null, signals: gradeSignals };
  }
  ...
  gradeSignals.push({
    ...
    value: reqFields ? 'Present' : 'Missing',
...
/**
 * PR-8: ABM / target account process (marketing + sales, TRANSCRIPT only)
 * No strong API signal - scored entirely from transcript/consultant
 */
function gradePR8(_s) {
...
function gradePR9(s) {
  ...
  let score;
  if (attribution && sourceTracking && campaigns >= 5) {
    score = 5;
  ...
  gradeSignals.push({
    ...
    value: sourceTracking ? 'Present' : 'Missing',
 ...
 */
function gradePR10(s) {
  const gradeSignals = [];
  const pipelines = s.deal_pipeline_stages;
  const hasPipeline = pipelines && pipelines.length > 0;
  const stalledNotif = !!s.has_stalled_deal_notification;

  if (pipelines === undefined && s.has_stalled_deal_notification === undefined) {
    return { score: null, signals: gradeSignals };
  }

  let score;
  if (hasPipeline && stalledNotif) {
    score = 4;
  } else if (hasPipeline) {
    score = 3;
  } else {
...
export function gradeProcess(signals, intakeAnswers, transcriptScores, consultantScores) {
  return PROCESS_COMPETENCIES.map((competency) => {
    // Step 1: Compute API-based score (same score for all applicable departments)
    const apiResult = competency.grade(signals);
