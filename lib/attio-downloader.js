...
export async function downloadAndStoreMetadata(customerId, workspaceId, accessToken) {
  ...
  // 2. Top-level resources in parallel. Attio paths use snake_case
  // (workspace_members, not workspace-members). Each non-critical fetch is
  // wrapped in .catch() so a missing scope / 404 doesn't fail the whole download.
  const [objectsRes, listsRes, workspaceMembersRes, webhooksRes] = await Promise.all([
    fetchJSON(`${API_BASE}/v2/objects`, accessToken),
    fetchJSON(`${API_BASE}/v2/lists`, accessToken).catch((err) => {
      console.warn('[attio-downloader] /v2/lists failed:', err.message);
      return { data: [] };
    }),
    fetchJSON(`${API_BASE}/v2/workspace_members`, accessToken).catch((err) => {
      console.warn('[attio-downloader] /v2/workspace_members failed:', err.message);
      return { data: [] };
    }),
    fetchJSON(`${API_BASE}/v2/webhooks`, accessToken).catch((err) => {
      console.warn('[attio-downloader] /v2/webhooks failed:', err.message);
      return { data: [] };
    }),
  ]);
